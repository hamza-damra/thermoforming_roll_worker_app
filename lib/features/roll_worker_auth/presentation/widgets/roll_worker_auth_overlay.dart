  ...
  void _onAuthStateChanged(BatchAuthState? prev, BatchAuthState next) {
    ...
    switch (next) {
      ...
      case BatchAuthFailure(:final AppFailure failure):
        // V109: login while a roll is mounted now succeeds on the normal auth
        // path (the backend no longer throws ROLL_WORKER_TAKEOVER_REQUIRED), so
        // every failure flows through the generic inline-error path below.
        _pinController.clear();
